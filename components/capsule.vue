<template>
	<div class="contenu">
		...
		<div class="media" v-if="item.media !== '' && action !== 'organiser'">
			...
			<span role="button" :tabindex="tabindex" v-else-if="item.type === 'audio' || item.type === 'video' || (item.type === 'document' && visionneuseDocx !== '') || item.type === 'pdf' || (item.type === 'office' && visionneuseDocx !== '') || item.type === 'embed'" @click="$parent.$parent.afficherVisionneuse(item)" @keydown.enter="$parent.$parent.afficherVisionneuse(item)"><img :class="{'vignette': $parent.$parent.definirVignette(item).substring(0, 5) !== '/img/'}" :src="$parent.$parent.definirVignette(item)" :alt="$parent.$parent.definirNomLienFichier($parent.$parent.definirVignette(item))" loading="lazy"></span>
