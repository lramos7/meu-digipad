<template>
	<div class="contenu">
		...
		<div class="media" v-if="item.media !== '' && action !== 'organiser'">
			...
			<span role="button" :tabindex="tabindex" v-else-if="item.type === 'audio' || item.type === 'video' || (item.type === 'document' && visionneuseDocx && visionneuseDocx !== '' && visionneuseDocx !== null) || item.type === 'pdf' || (item.type === 'office' && visionneuseDocx && visionneuseDocx !== '' && visionneuseDocx !== null) || item.type === 'embed'" @click="$parent.afficherVisionneuse(item)" @keydown.enter="$parent.afficherVisionneuse(item)"><img :class="{'vignette': $parent.definirVignette(item).substring(0, 5) !== '/img/'}" :src="$parent.definirVignette(item)" :alt="$parent.definirNomLienFichier($parent.definirVignette(item))" loading="lazy"></span>
